...
class ChatSockets:
    ...
    def handleChatWSFrames(self,tcpHandler,username,DB,MUTEX):
        global sockets
        ...
        while True:
            with MUTEX:
                ...
                if len(buffer) > 0:
                    ...
                    break
    def handle_socket(self,tcpHandler,username,DB,MUTEX):
        global sockets
        handler = {'handler':tcpHandler,'username':username}
        sockets.append(handler)
        self.handleChatWSFrames(tcpHandler,username,DB,MUTEX)
